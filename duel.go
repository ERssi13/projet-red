...
// Définir une structure pour les personnages (joueur et gobelin)
...
// Fonction principale du combat d'entraînement
func trainingFight(player *Character, goblin *Character, items []Item) {
	for player.Health > 0 && goblin.Health > 0 {
		fmt.Println("Début du tour:")
		fmt.Println("1. CharTurn")
		fmt.Println("2. GoblinPattern")
...
func projetred() {
	...
	goblin := Character{Name: "Gobelin d'entraînement", Health: 80, Attack: 10}

	// Exemples d'objets avec effets
	items := []Item{
		{Name: "Potion de soin", Effect: func(c *Character) {
			c.Health += 10
			if c.Health > 100 {
				c.Health = 100
			}
